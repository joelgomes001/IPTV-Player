document.addEventListener('DOMContentLoaded', () => {
  ...
  // Load Base Channels and Sync Live with both Rana's API Server & IPTV-Org Database
  function loadLiveChannels() {
    ...
    fetch(`channels.json?_t=${Date.now()}`, { cache: 'no-cache' })
      ...
      .then(baseChannels => {
        ...
        statusBadge.innerHTML = `<span style="color:#ffffff;">● ${allChannels.length.toLocaleString()} Channels Loaded</span>`;
        ...
        baseChannels.forEach(c => {
          if (c.stream_url) channelMap.set(c.stream_url.trim(), c);
        });

        // 1. Sync with Rana's API
        const ranaPromise = (async () => {
          const targetUrl = `${BASE_API_URL}all_tv_channel_by_category`;
          const proxyUrl = `https://corsproxy.io/?${encodeURIComponent(targetUrl)}`;
          const res = await fetch(proxyUrl, { headers: { 'API-KEY': API_KEY } });
        ...
        const iptvOrgInPromise = (async () => {
          const res = await fetch('https://iptv-org.github.io/iptv/countries/in.m3u');
          if (!res.ok) throw new Error('IPTV-Org India M3U error');
          const text = await res.text();
          const parsed = parseM3uToChannels(text, 'Indian');
          let added = 0;
          parsed.forEach(c => {
            if (!channelMap.has(c.stream_url)) {
              channelMap.set(c.stream_url, c);
              added++;
            }
          });
          return added;
        ...
        const iptvOrgIndexPromise = (async () => {
          const res = await fetch('https://iptv-org.github.io/iptv/index.m3u');
          if (!res.ok) throw new Error('IPTV-Org Index M3U error');
          const text = await res.text();
          const parsed = parseM3uToChannels(text, 'International');
          let added = 0;
          parsed.forEach(c => {
            if (!channelMap.has(c.stream_url)) {
              channelMap.set(c.stream_url, c);
        ...
        Promise.allSettled([ranaPromise, iptvOrgInPromise, iptvOrgIndexPromise])
          .then(() => {
            const initialCount = baseChannels.length;
            allChannels = Array.from(channelMap.values());
            const newChannelsCount = allChannels.length - initialCount;
            ...
            if (newChannelsCount > 0) {
              showToast(`Synced ${newChannelsCount.toLocaleString()} new channels!`);
            } else {
              showToast(`Sync complete! All ${allChannels.length.toLocaleString()} channels active.`);
            }
          });
      })
      .catch(e => {
  ...
  function filterAndRender() {
    if (searchInput) searchInput.placeholder = 'Search channels...';
    ...
    if (currentFilteredChannels.length === 0) {
      const msg = searchInput.value.trim() 
        ? `No channels found matching "${searchInput.value.trim()}". Try searching another keyword.`
